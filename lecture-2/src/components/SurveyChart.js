...
const SurveyChart = () => {
  ...
  const toggleFilter = (questionIndex, itemIndex) => {
    console.log('questiionIndex, itemIndex: ', questionIndex, itemIndex);
    let _filter = Object.assign({}, filter);
    if (_filter.hasOwnProperty(questionIndex) && _filter[questionIndex] === itemIndex) {
      console.log('[삭제]questiionIndex, itemIndex: ', questionIndex, itemIndex);
      delete _filter[questionIndex];
    } else {
      console.log('[추가]questiionIndex, itemIndex: ', questionIndex, itemIndex);
